...
- **AUDIT — 2026-09-02 (read this before any code change):**
  - External three-document audit asserted 16 findings. All 16 verified and confirmed; none contradicted; two revised upward in scope. 19 further findings recorded (NF-1 to NF-19), six critical.
  ...
  - **NF-17 (CRITICAL) — L2c's workflow shell-exec signal is keyed on the wrong axis.** The `curl|bash` payload is scored *only* when paired with a dormant trigger (`workflow_dispatch`/`schedule`). Same payload on `on: push` or `on: pull_request` → **no signal at all, SAFE**. The immediately-executing variant scores lower than the dormant one. Fix: standalone payload signal independent of trigger; trigger becomes a severity modifier (auto-firing escalates), never a detection gate. FP control via an installer-host allowlist mirroring `trusted_oidc_consumers`.
  - **NF-18 (HIGH) — L2c does not scan composite actions at their conventional paths.** `_ACTIONS_WORKFLOW_PATTERN` requires the file directly under `.github/workflows/` or `.github/actions/`, so `.github/actions/<name>/action.yml` (the documented layout) and a repo-root `action.yml` (what a published action uses, including this one) are both missed. They fall through to L2's content scan: REVIEW (2.0), never blocking. Fixing NF-15/NF-17 widened the gap from one verdict band to three — identical bytes are DESTRUCTIVE or REVIEW by directory depth alone. Found by dogfooding; the analyser has never scanned its own `action.yml`.
  - **NF-19 (HIGH) — normalisation turns line patterns into file patterns.** `_normalize_yaml_content` strips every newline, and L2c matches several families against that output as well as the raw file, so `[^\n]*` silently changes from "elsewhere on this line" to "anywhere in the file". This repo's own `action.yml` (194 lines → one 6,125-char line) matched `credential_harvest` by stitching an `env:` block, an unrelated `| sed`, and the substring `KEY` inside `PAYLOADGUARD_PRIVATE_KEY`. CRITICAL → DESTRUCTIVE → **exit 2: a blocking false positive**, worse than NF-16 which caps at CAUTION. Fixed on the A1 branch by bounding the wildcards; the principled fix is scoping matches to a single `run:` block, which needs structural parsing.
  - **NF-19 recurred in the NF-17 patterns written the same session.** A benign `curl … -o file` plus an unrelated `cat local.sh | bash` four steps later matched as remote execution. Any new pattern matched against normalised content must carry a bounded quantifier.
  - **Fixture decisions settled 2026-09-03:** `RTA03` → DESTRUCTIVE (discriminator is the untrusted checkout, not permissions). `AW02` stays CAUTION — verified no PR-head checkout, it is the FP anchor. `WS03` and `RTA04` → DESTRUCTIVE via NF-17. New fixture needed for `on: push` + `curl|bash`, SAFE today.
