...
A live incident (2026-04-24) demonstrated this vector. A weaponized PDF processed by the developer's notebook LLM induced context collapse, causing the model to produce a "Technical Remediation Report" as its compromised output. The developer committed this output to main unintentionally, not recognising it as the product of an attack. The report contained embedded CI trigger strings (`[citest commit:<sha>]`), filesystem privilege escalation commands (`setfacl`), and plausible-looking Go remediation code. PayloadGuard scored the commit as low-risk (pure file addition, no deletions). Human code review caught it. The attacker never interacted with the repository directly — the developer was the unwitting delivery mechanism.
